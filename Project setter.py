import os
import shutil
from PIL import Image, ImageTk
try:
    import Tkinter as tk
    from Tkinter import Filedialog as filedialog
    import tkMessageBox as messagebox

except ImportError:
    import tkinter as tk
    from tkinter import filedialog
    from tkinter import messagebox
        
HEIGHT = 400
WIDTH = 1000

def create_file(entry):
    name = entry
    path = str(filedialog.askdirectory())
    print(name, path)
    try:
		source = r'Z:\Vorlagen\Pr_Nr_Ordner_Struktur'
		for data_stucture in os.listdir(source):
			shutil.copytree(os.path.join(source, data_stucture), os.path.join(path, data_stucture))

		bearbeitung = path + r'\5_Bearbeitung'
		GIS = bearbeitung + r'\GIS'
		WWK = 'Z:\Vorlagen\WWK'
		shutil.copytree(WWK, GIS)
		# list files in the folder
		WWK_list = os.listdir(GIS)

		for file in WWK_list:
			print(file)
			full = os.path.join(GIS, file)
			new_full = full.replace('Kommune', name)
			os.rename(full, new_full)

		messagebox.showinfo('Info', 'Done!')
    except WindowsError:
	    messagebox.showerror('Error', 'file already exist')

root = tk.Tk()

canvas = tk.Canvas(root, height=HEIGHT, width=WIDTH)
canvas.pack()

load = Image.open("Logo2.png")
render = ImageTk.PhotoImage(load)
img = tk.Label(root, image=render)
img.image = render
img.place(x=0, y=0, relwidth=1, relheight=1)


frame = tk.Frame(root, bg='#80c1ff', bd=5)
frame.place(relx=0.5, rely=0.1, relwidth=0.75, relheight=0.2, anchor='n')

Input_Label = tk.Label(frame, text='Input Kommun name', bd=5,bg='#80c1ff', width=40, font=40).place(relx=0.05, relwidth=0.45, relheight=1)
entry = tk.Entry(frame, font=40)
entry.place(relx=0.55,relwidth=0.4, relheight=1)

lower_frame = tk.Frame(root, bg='#80c1ff', bd=10)
lower_frame.place(relx=0.5, rely=0.75, relwidth=0.75, relheight=0.2, anchor='n')

create_button = tk.Button(lower_frame,text='Create Files', font = 40, command= lambda:create_file(entry.get()))
create_button.place(relx=0.5, relheight=1, relwidth=0.8, anchor='n')
root.mainloop()
